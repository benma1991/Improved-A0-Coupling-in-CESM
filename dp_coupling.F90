...
chnk_loop1 : &
    ...
    !----------------added by Ma--------------------------------
                phys_state(lchnk)%ud   (i,k) = u3sxy(ic,jc,k)
                phys_state(lchnk)%vd   (i,k) = v3sxy(ic,jc,k)
    !-----------------------------------------------------------
...
!dir$ concurrent
        ...
        !----------added by Ma---------------------------
                bbuffer(bpter(ib,k)+4) = u3sxy   (i,j,k)
                bbuffer(bpter(ib,k)+5) = v3sxy    (i,j,k)
        !------------------------------------------------
        !----------changed by Ma-------------------------
                ...
                !end if
                bbuffer(bpter(ib,k)+6) = omgaxy(i,k,j)
                bbuffer(bpter(ib,k)+7) = ptxy(i,j,k) / (D1_0 + zvir*tracer(i,j,k,1))
                bbuffer(bpter(ib,k)+8) = pkxy(i,j,pver+1) / pkzxy(i,j,k) 
                if (do_waccm_phys()) then
       ...
       do lchnk = begchunk,endchunk
          ...
          do i=1,ncol
             ...
             do k=1,km
                ...
                !---------added by Ma--------------------------
                phys_state(lchnk)%ud     (i,k) = cbuffer(cpter(i,k)+4)
                phys_state(lchnk)%vd     (i,k) = cbuffer(cpter(i,k)+5)
                !----------------------------------------------
                !----------changed by Ma-----------------------
                ...
                !end if
                phys_state(lchnk)%omega (i,k) = cbuffer(cpter(i,k)+6)
                phys_state(lchnk)%t     (i,k) = cbuffer(cpter(i,k)+7)
                phys_state(lchnk)%exner (i,k) = cbuffer(cpter(i,k)+8)
                if (do_waccm_phys()) then
