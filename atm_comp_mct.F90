  ...
  subroutine atm_export_mct( cam_out, a2x_a )
    ...
    do c=begchunk, endchunk
       ...
       do i=1,ncols
          ...
          !----------added by Ma----------------------------------
          a2x_a%rAttr(index_a2x_Sa_ud       ,ig) = cam_out(c)%udbot(i)   
          a2x_a%rAttr(index_a2x_Sa_vd       ,ig) = cam_out(c)%vdbot(i)
          a2x_a%rAttr(index_a2x_Sa_du_ptend ,ig) = cam_out(c)%dubot_ptend(i)   
          a2x_a%rAttr(index_a2x_Sa_dv_ptend ,ig) = cam_out(c)%dvbot_ptend(i)
          !-------------------------------------------------------
