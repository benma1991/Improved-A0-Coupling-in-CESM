...
#include <mpif.h>
   ...
   !----------------------------------------------------------------------------
   !----added by Ma-------------------------------------
   !   _M are added new mapper for ud
   !   _N are added new mapper for vd
   !----------------------------------------------------
   ...
   !----------added by Ma------------------
   type(seq_map), SAVE :: mapper_Ma2o
   type(seq_map), SAVE :: mapper_Na2o
   !---------------------------------------
...
!   endif
   ...
   if (iamin_CPLID) then
      ...
      if (ocn_present) then
         ...
         !----------------added by Ma-----------------------------------------
         if (iamroot_CPLID) write(logunit,*) ' '
         if (iamroot_CPLID) write(logunit,F00) 'Initializing mapper_Ma2o'
         call seq_map_init_rcfile(mapper_Ma2o,gsmap_ax,gsmap_ox,mpicom_CPLID, &
             'seq_maps.rc','atm2ocn_mmapname:','atm2ocn_mmaptype:',samegrid_ao, &
             'mapper_Ma2o initialization',esmf_map_flag)
         if (iamroot_CPLID) write(logunit,*) ' '
         if (iamroot_CPLID) write(logunit,F00) 'Initializing mapper_Na2o'
         call seq_map_init_rcfile(mapper_Na2o,gsmap_ax,gsmap_ox,mpicom_CPLID, &
             'seq_maps.rc','atm2ocn_nmapname:','atm2ocn_nmaptype:',samegrid_ao, &
             'mapper_Na2o initialization',esmf_map_flag)
         !--------------------------------------------------------------------
         if (iamroot_CPLID) write(logunit,*) ' '
         ...
         !--------------------------------added by Ma----------------------------
         if (iamroot_CPLID) write(logunit,*) ' '
         if (iamroot_CPLID) write(logunit,F00) 'Initializing mapper_Ma2o vect'
         call seq_map_initvect(mapper_Ma2o,vect_map,dom_ax,dom_ox, &
              gsmap_s=gsmap_ax,ni=atm_nx,nj=atm_ny,string='mapper_Ma2o initvect')
         
         if (iamroot_CPLID) write(logunit,*) ' '
         if (iamroot_CPLID) write(logunit,F00) 'Initializing mapper_Na2o vect'
         call seq_map_initvect(mapper_Na2o,vect_map,dom_ax,dom_ox, &
              gsmap_s=gsmap_ax,ni=atm_nx,nj=atm_ny,string='mapper_Na2o initvect')
         !-----------------------------------------------------------------------
   ...
   do while ( .not. stop_alarm)
      ...
      if (iamin_CPLID .and. (ice_present.or.ocn_present) .and. atm_present) then
         ...
         do eai = 1,num_inst_atm
            ...
            !!--- tcx this Va2o call will not be necessary when npfix goes away
            call seq_map_map(mapper_Va2o, a2x_ax(eai), a2x_ox(eai), fldlist='Sa_du_ptend:Sa_dv_ptend', norm=.true.)
            !!--- tcx the norm should be true below, it's false for bfb backwards compatability
!            call seq_map_mapvect(mapper_Va2o,vect_map,a2x_ax(eai),a2x_ox(eai),'Sa_du_ptend','Sa_dv_ptend',norm=.false.)

            !--- tcx this Va2o call will not be necessary when npfix goes away
            call seq_map_map(mapper_Ma2o, a2x_ax(eai), a2x_ox(eai), fldlist='Sa_ud', norm=.true.)
            !--- tcx the norm should be true below, it's false for bfb backwards compatability
!            call seq_map_mapvect(mapper_Ma2o,vect_map,a2x_ax(eai),a2x_ox(eai),'Sa_u','Sa_v',norm=.false.)

            call seq_map_map(mapper_Na2o, a2x_ax(eai), a2x_ox(eai), fldlist='Sa_vd', norm=.true.)
            !call seq_map_mapvect(mapper_Na2o,vect_map,a2x_ax(eai),a2x_ox(eai),'Sa_u','Sa_v',norm=.false.)
            call mct_aVect_add(a2x_ox(eai), fld1='Sa_ud', fld2='Sa_du_ptend', fld_add='Sa_u')
            call mct_aVect_add(a2x_ox(eai), fld1='Sa_vd', fld2='Sa_dv_ptend', fld_add='Sa_v')
            !------------------------------------------------------------------------------------------------
         enddo
