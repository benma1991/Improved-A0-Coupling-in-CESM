...
subroutine diag_init()
  ...
  ! Declare the history fields for which this module contains outfld calls.
   ...
   !---------added by Ma---------------------------------
   call addfld ('DU_PT   ','m/s     ',pver, 'A','Delt Zonal wind due to physical tendency',phys_decomp)
   call addfld ('DV_PT   ','m/s     ',pver, 'A','Delt Meridional wind due to physical tendency',phys_decomp)
   call addfld ('UD      ','m/s     ',pver, 'A','Zonal wind on stagged grid',phys_decomp)
   call addfld ('VD      ','m/s     ',pver, 'A','Meridional wind on stagged grid',phys_decomp)
   !--------------------------------------------------
   ...
   if (history_amwg) then
      ...
      !-------added by Ma---------------------------
      call add_default ('DU_PT   '  , 1, ' ')
      call add_default ('DV_PT   '  , 1, ' ')
      call add_default ('UD      '  , 1, ' ')
      call add_default ('VD      '  , 1, ' ')
      !---------------------------------------------
   ...
   if ( history_budget ) then
      ...
      !------------------added by Ma----------------------------------------
      call add_default ('DU_PT   '  , history_budget_histfile_num, ' ')
      call add_default ('DV_PT   '  , history_budget_histfile_num, ' ')
      call add_default ('UD      '  , history_budget_histfile_num, ' ')
      call add_default ('VD      '  , history_budget_histfile_num, ' ')
      !---------------------------------------------------------------------
...
!
    ...
    !------------added by Ma--------------------------
    call outfld('DU_PT   ',state%du_ptend , pcols   ,lchnk   )
    call outfld('DV_PT   ',state%dv_ptend , pcols   ,lchnk   )
    call outfld('UD      ',state%ud, pcols   ,lchnk   )
    call outfld('VD      ',state%vd, pcols   ,lchnk   )
    !--------------------------------------------------
    do m=1,pcnst
