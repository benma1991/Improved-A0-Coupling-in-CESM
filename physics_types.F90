...
!-------------------------------------------------------------------------------
  type physics_state
     ...
     real(r8), dimension(:,:),allocatable        :: &
          ...
          !------------added by Ma---------------------------------------
          ud,      &! zonal wind at d grid (m/s)
          vd,      &! meridional wind at d grid (m/s)
          du_ptend,&! delt u accumulation of physical tendency(du=ptend%u*dt,m/s)
          dv_ptend,&! delt v accumulation of physical tendency(dv=ptend%v*dt,m/s)
          !--------------------------------------------------------------
...
!------------------changed by Ma----------------------------------
    ...
    subroutine physics_update(state, ptend, dt, tend, flag)
!-----------------------------------------------------------------------
...
!---------------------------Local storage-------------------------------
    ...
    if(ptend%lu) then
       do k = ptend%top_level, ptend%bot_level
          ...
          if (present(flag)) then
              if (flag.eq.1) &
                  state%du_ptend (:ncol,k) = state%du_ptend(:ncol,k) &
                                             + ptend%u(:ncol,k) * dt
          else
              state%du_ptend (:ncol,k) = state%du_ptend (:ncol,k) &
                                             + ptend%u(:ncol,k) * dt
    ...
    if(ptend%lv) then
       do k = ptend%top_level, ptend%bot_level
          ...
          if (present(flag)) then
              if (flag.eq.1) &
                  state%dv_ptend (:ncol,k) = state%dv_ptend(:ncol,k) &
                                             + ptend%v(:ncol,k) * dt
          else
              state%dv_ptend (:ncol,k) = state%dv_ptend (:ncol,k) &
                                             + ptend%v(:ncol,k) * dt
...
!-----------------------------------------------------------------------
    ...
    !---------------------added by Ma-------------------------------------
    call shr_assert_in_domain(state%ud(:ncol,:),        is_nan=.false., &
         varname="state%ud",         msg=msg)
    call shr_assert_in_domain(state%vd(:ncol,:),        is_nan=.false., &
         varname="state%vd",         msg=msg)
    call shr_assert_in_domain(state%du_ptend(:ncol,:),  is_nan=.false., &
         varname="state%du_ptend",         msg=msg)
    call shr_assert_in_domain(state%dv_ptend(:ncol,:),  is_nan=.false., &
         varname="state%dv_ptend",         msg=msg)
    !---------------------------------------------------------------------
    ...
    call shr_assert_in_domain(state%v(:ncol,:),         lt=posinf_r8, gt=neginf_r8, &
         varname="state%v",         msg=msg)
    !---------------------------added by Ma-------------------------------------
    call shr_assert_in_domain(state%ud(:ncol,:),        lt=posinf_r8, gt=neginf_r8, &
         varname="state%ud",        msg=msg)
    call shr_assert_in_domain(state%vd(:ncol,:),        lt=posinf_r8, gt=neginf_r8, &
         varname="state%vd",        msg=msg)
    call shr_assert_in_domain(state%du_ptend(:ncol,:),  lt=posinf_r8, gt=neginf_r8, &
         varname="state%du_ptend",  msg=msg)
    call shr_assert_in_domain(state%dv_ptend(:ncol,:),  lt=posinf_r8, gt=neginf_r8, &
         varname="state%dv_ptend",  msg=msg)
    !---------------------------------------------------------------------------
  ...
  subroutine physics_state_copy(state_in, state_out)
    ...
    do k = 1, pver
       do i = 1, ncol
          ...
          !--------------------added by Ma---------
          state_out%ud(i,k)        = state_in%ud(i,k) 
          state_out%vd(i,k)        = state_in%vd(i,k) 
          state_out%du_ptend(i,k)  = state_in%du_ptend(i,k)
          state_out%dv_ptend(i,k)  = state_in%dv_ptend(i,k)
          !----------------------------------------
...
! allocate the individual state components
  ...
  !-------------------------added by Ma--------------------------
  allocate(state%ud(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%ud')
  
  allocate(state%vd(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%vd')

  allocate(state%du_ptend(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%du_ptend')
  
  allocate(state%dv_ptend(psetcols,pver), stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_alloc error: allocation error for state%dv_ptend')
  !--------------------------------------------------------------
...
! deallocate the individual state components
  ...
  !---------------------added by Ma---------------------------------------
  deallocate(state%ud, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%ud')
  
  deallocate(state%vd, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%vd')

  deallocate(state%du_ptend, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%du_ptend')
  
  deallocate(state%dv_ptend, stat=ierr)
  if ( ierr /= 0 ) call endrun('physics_state_dealloc error: deallocation error for state%dv_ptend')
  !-----------------------------------------------------------------------
