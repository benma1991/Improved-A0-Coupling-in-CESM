...
!---------------------------------------------------------------------------
  ...
  !---------------added by Ma---------------------------------
     real(r8) :: udbot(pcols)        ! bot level u wind at d grid
     real(r8) :: vdbot(pcols)        ! bot level v wind at d grid
     real(r8) :: dubot_ptend(pcols)  ! bot level delt u
     real(r8) :: dvbot_ptend(pcols)  ! bot level delt v
  !-----------------------------------------------------------
...
!
    ...
    !-------added by Ma---------------------------
       cam_out(c)%udbot(:)     = 0._r8
       cam_out(c)%vdbot(:)     = 0._r8
       cam_out(c)%dubot_ptend(:)     = 0._r8
       cam_out(c)%dvbot_ptend(:)     = 0._r8
    !---------------------------------------------
...
!-----------------------------------------------------------------------
   ...
   do i=1,ncol
      ...
      !-----------added by Ma----------------------
      cam_out%udbot(i)  = state%ud(i,pver)
      cam_out%vdbot(i)  = state%vd(i,pver)
      cam_out%dubot_ptend(i)  = state%du_ptend(i,pver)
      cam_out%dvbot_ptend(i)  = state%dv_ptend(i,pver)
      !--------------------------------------------
