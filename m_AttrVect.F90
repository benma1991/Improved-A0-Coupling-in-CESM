...
!------------------------added by Ma------------------
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine Add_(av,fld1,fld2,fld_add)
  implicit none 
  !-----------------------------------------------------
  !    
  ! Arguments
  ...
  character(len=*),intent(in) :: fld2
  character(len=*),intent(in) :: fld_add
  !    
  ! Local Variables
  integer :: j, lsize_v
  integer :: ind1, ind2, ind_add
  type(AttrVect) :: tempaV

  !--- formats ---
  character(len=*),parameter :: myname_=myname//'::Add_'

  ind1 = indexRA_(av,fld1)
  ind2 = indexRA_(av,fld2)
  ind_add = indexRA_(av,fld_add)
    
  lsize_v = lsize_(av)

! create and fill a temporary AttrVect to hold any data currently in the aV
  call initv_(tempaV,aV,lsize=lsize_v)
  call Copy_(aV,tempaV)

!$OMP PARALLEL DO PRIVAtE(j)
  do j = 1, lsize_v
     !av%rAttr(ind_add,j)=0.0_r8
     tempaV%rAttr(ind_add,j)=tempaV%rAttr(ind1,j) + tempaV%rAttr(ind2,j)
  end do
